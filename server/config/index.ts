import { Config, ContentTypeConfig } from 'strapi-plugin-deepcopy/config'
...
export default {
  ...
  validator: (config) => {
    ...
    for (let i = 0; i < Object.entries(config.contentTypes).length; i += 1) {
      ...
      const { editableFields, uniqueFields } = contentTypeConfig as any as ContentTypeConfig
      if (editableFields) {
        for (let j = 0; i < Object.entries(editableFields).length; i += 1) {
          ...
          if (typeof editableField !== 'object') {
            throw new Error(
              `contentTypes.${contentType}.editableFields.${fieldName} has to be an object`,
            )
          }

          if (typeof editableField.initialValue !== 'function') {
            throw new Error(
              `contentTypes.${contentType}.editableFields.${fieldName}.initialValue has to be a function`,
            )
          }
        }
      }
      ...
      if (uniqueFields) {
        for (let j = 0; i < Object.entries(uniqueFields).length; i += 1) {
          ...
          if (typeof editableField !== 'object') {
            throw new Error(
              `contentTypes.${contentType}.uniqueFields.${fieldName} has to be an object`,
            )
          }

          if (typeof editableField.value !== 'function') {
            throw new Error(
              `contentTypes.${contentType}.uniqueFields.${fieldName}.value has to be a function`,
            )
          }
        }
      }
      ...
      if (editableFields && uniqueFields) {
        ...
        const intersection = editableFieldNames.filter((fieldName) =>
